...
if (isset($_GET['action'])) {
    ...
    // =====================================================================
        ...
        // =====================================================================
        $where_volume = [];
        $joins_volume = "FROM Shipping s
                        INNER JOIN Company d ON d.CompanyID = s.DistributorID";

        // REGION filter takes priority
        if (!empty($region)) {
            // Additional JOINs needed to access location data
            $joins_volume .= " INNER JOIN Company src ON src.CompanyID = s.SourceCompanyID
        ...
        // =====================================================================
        $sql_volume = "SELECT d.CompanyName AS DistributorName, SUM(s.Quantity) AS TotalQty
                    $joins_volume
                    $where_clause_volume
                    GROUP BY s.DistributorID, d.CompanyName
        ...
        $where_distributors = [];
        $joins_distributors = "FROM Shipping s
                            INNER JOIN Distributor dist ON s.DistributorID = dist.CompanyID";

        // Same filter logic as Query 1
        if (!empty($region)) {
            $joins_distributors .= " INNER JOIN Company src ON src.CompanyID = s.SourceCompanyID
                                    INNER JOIN Location l ON l.LocationID = src.LocationID";
        ...
        // =====================================================================
        $sql_delivery = "SELECT d.CompanyName AS DistributorName,
                        SUM(s2.ActualDate <= s2.PromisedDate) * 100.0 / COUNT(*) AS DeliveryRate
                        FROM (
        ...
        $where_status = [];
        $joins_status = "FROM Shipping s
                        LEFT JOIN Receiving r ON r.ShipmentID = s.ShipmentID";

        if (!empty($region)) {
            $joins_status .= " INNER JOIN Company src ON src.CompanyID = s.SourceCompanyID
                            INNER JOIN Location l ON l.LocationID = src.LocationID";
        ...
        $sql_products = "SELECT c.CompanyName AS DistributorName, p.ProductID, p.ProductName, p.Category,
                        COUNT(*) AS ShipmentCount,
        ...
        if (!empty($region)) {
            ...
            $where_all_trans[] = "(ls.ContinentName = '$region' OR lr.ContinentName = '$region' OR la.ContinentName = '$region')";
        }
        elseif (!empty($start_date) && !empty($end_date)) {
            // Check dates across all transaction types
            $where_all_trans[] = "(
                (s.PromisedDate BETWEEN '$start_date' AND '$end_date') OR
                (r.ReceivedDate BETWEEN '$start_date' AND '$end_date') OR
                (a.AdjustmentDate BETWEEN '$start_date' AND '$end_date')
            )";
        }
        ...
        if (!empty($source_company)) {
            $where_all_trans[] = "(
                (it.Type = 'Shipping' AND s.SourceCompanyID = '$source_company') OR
                (it.Type = 'Adjustment' AND ca.CompanyID = '$source_company')
            )";
        }

        if (!empty($destination_company)) {
            $where_all_trans[] = "(
                (it.Type = 'Receiving' AND cr.CompanyID = '$destination_company') OR
                (it.Type = 'Adjustment' AND ca.CompanyID = '$destination_company')
            )";
        }

        $where_clause_all_trans = count($where_all_trans) > 0 ? "WHERE " . implode(" AND ", $where_all_trans) : "";

        // =====================================================================
        // LARGE SELECT WITH ALIASED COLUMNS
        // Using AS to create descriptive column names for frontend processing
        // =====================================================================
        $sql_all_trans = "SELECT it.TransactionID, it.Type,
                        s.ShipmentID AS Shipping_ShipmentID, cs.CompanyName AS Shipping_SourceCompany, 
                        cd.CompanyName AS Shipping_DestinationCompany, ps.ProductName AS Shipping_Product,
                        s.PromisedDate AS Shipping_PromisedDate, s.ActualDate AS Shipping_ActualDate, s.Quantity AS Shipping_Quantity,
                        r.ReceivingID AS Receiving_ReceivingID, r.ShipmentID AS Receiving_ShipmentID,
                        cr.CompanyName AS Receiving_Company, r.ReceivedDate AS Receiving_ReceivedDate, r.QuantityReceived AS Receiving_Quantity,
                        a.AdjustmentID AS Adjustment_AdjustmentID, ca.CompanyName AS Adjustment_Company,
                        pa.ProductName AS Adjustment_Product, a.AdjustmentDate AS Adjustment_Date,
                        a.QuantityChange AS Adjustment_QuantityChange, a.Reason AS Adjustment_Reason
                        $joins_all_trans
...
<nav>
    ...
    <div style="display: flex; align-items: center; gap: 6px;">
        ...
        <input type="date" id="transaction-start-date" style="width: 130px; height: 34px; padding: 6px 10px; font-size: 13px; border: 1px solid #999; border-radius: 4px;">
        <input type="date" id="transaction-end-date" style="width: 130px; height: 34px; padding: 6px 10px; font-size: 13px; border: 1px solid #999; border-radius: 4px;">
        ...
        <div class="search-box-two" style="position: relative; width: 120px;">
            <input type="text" id="source-company-input" placeholder="Company (leaving)" autocomplete="off" style="height: 34px;">
            <div class="search-results" id="source-company-results"></div>
        </div>
        
        <!-- Destination company search -->
        <div class="search-box-two" style="position: relative; width: 120px;">
            <input type="text" id="destination-company-input" placeholder="Company (arriving)" autocomplete="off" style="height: 34px;">
...
<!-- ALL TRANSACTIONS - Fixed height with scrolling -->
    <div class="card" style="padding: 15px; margin-bottom: 15px; height: 180px;">
        <h3 style="font-size: 14px; font-weight: 600; margin-bottom: 10px; color: #333;">All Transactions (Detailed View)</h3>
        <!-- Div for dynamic content population - innerHTML usage from JS lab problem 9 -->
        <div id="data-all-transactions" style="font-size: 10px; height: 280px; overflow-y: auto;">
...
<div class="card" style="padding: 12px; margin-bottom: 15px; height: 180px; overflow: visible;">
    <div style="display: grid; grid-template-columns: repeat(5, 1fr); gap: 15px; height: 100%;">
        
        <!-- Each data box follows same pattern -->
        <div style="display: flex; flex-direction: column;">
            <h3 style="font-size: 12px; font-weight: 600; margin-bottom: 6px; color: #333;">Shipment Volume</h3>
            <!-- Independent scrolling containers - CSS overflow property -->
            <div id="data-shipment-volume" style="font-size: 10px; height: 140px; overflow-y: auto; overflow-x: hidden;">
                <p style="color: #999;">No data</p>
            </div>
        </div>

        <div style="display: flex; flex-direction: column;">
            <h3 style="font-size: 12px; font-weight: 600; margin-bottom: 6px; color: #333;">On-Time Delivery Rate</h3>
            <div id="data-delivery-rate" style="font-size: 10px; height: 140px; overflow-y: auto; overflow-x: hidden;">
                <p style="color: #999;">No data</p>
            </div>
        </div>

        <div style="display: flex; flex-direction: column;">
            <h3 style="font-size: 12px; font-weight: 600; margin-bottom: 6px; color: #333;">Shipment Status</h3>
            <div id="data-status" style="font-size: 10px; height: 140px; overflow-y: auto; overflow-x: hidden;">
                <p style="color: #999;">No data</p>
            </div>
        </div>

        <div style="display: flex; flex-direction: column;">
            <h3 style="font-size: 12px; font-weight: 600; margin-bottom: 6px; color: #333;">Products Handled</h3>
            <div id="data-products" style="font-size: 10px; height: 140px; overflow-y: auto; overflow-x: hidden;">
                <p style="color: #999;">No data</p>
            </div>
        </div>

        <div style="display: flex; flex-direction: column;">
            <h3 style="font-size: 12px; font-weight: 600; margin-bottom: 6px; color: #333;">Disruption Exposure</h3>
            <div id="data-disruption-exposure" style="font-size: 10px; height: 140px; overflow-y: auto; overflow-x: hidden;">
...
<script>
    ...
    document.addEventListener("click", function(event) {
        ...
        if (!event.target.closest("#region-search-input") && !event.target.closest("#region-search-results")) {
            regionSearchResults.classList.remove("show");
        }
        if (!event.target.closest("#source-company-input") && !event.target.closest("#source-company-results")) {
            sourceCompanyResults.classList.remove("show");
        }
        if (!event.target.closest("#destination-company-input") && !event.target.closest("#destination-company-results")) {
            destinationCompanyResults.classList.remove("show");
        }
    });
...
function displayData(data) {
    ...
    if(data.shipment_volume && data.shipment_volume.length > 0) {
        var html = '<p style="font-size: 10px; color: #666; margin-bottom: 5px; font-style: italic;">' + filterDesc + '</p>';
        html += '<table style="width:100%; border-collapse: collapse;">';
        html += '<tr style="background:#f5f5f5;"><th style="padding:4px; text-align:left; font-size:10px;">Distributor</th><th style="padding:4px; text-align:right; font-size:10px;">Qty</th></tr>';
        
        // Loop through data array
        for(var i = 0; i < data.shipment_volume.length; i++) {
            html += '<tr><td style="padding:4px; border-top:1px solid #eee;">' + data.shipment_volume[i].DistributorName + '</td>';
            html += '<td style="padding:4px; border-top:1px solid #eee; text-align:right;">' + data.shipment_volume[i].TotalQty + '</td></tr>';
        }
        html += '</table>';
        document.getElementById('data-shipment-volume').innerHTML = html;
    } else {
        document.getElementById('data-shipment-volume').innerHTML = '<p style="color: #999;">No data</p>';
    }
    ...
    if(data.delivery_rate && data.delivery_rate.length > 0) {
        var html = '<p style="font-size: 10px; color: #666; margin-bottom: 5px; font-style: italic;">' + filterDesc + '</p>';
        html += '<table style="width:100%; border-collapse: collapse;">';
        html += '<tr style="background:#f5f5f5;"><th style="padding:4px; text-align:left; font-size:10px;">Distributor</th><th style="padding:4px; text-align:right; font-size:10px;">Rate</th></tr>';
        
        for(var i = 0; i < data.delivery_rate.length; i++) {
            var rate = parseFloat(data.delivery_rate[i].DeliveryRate).toFixed(1);
            html += '<tr><td style="padding:4px; border-top:1px solid #eee;">' + data.delivery_rate[i].DistributorName + '</td>';
            html += '<td style="padding:4px; border-top:1px solid #eee; text-align:right;">' + rate + '%</td></tr>';
        }
        html += '</table>';
        document.getElementById('data-delivery-rate').innerHTML = html;
    } else {
        document.getElementById('data-delivery-rate').innerHTML = '<p style="color: #999;">No data</p>';
    }
    ...
    if(data.shipment_status && data.shipment_status.length > 0) {
        var html = '<p style="font-size: 10px; color: #666; margin-bottom: 5px; font-style: italic;">' + filterDesc + '</p>';
        html += '<table style="width:100%; border-collapse: collapse;">';
        html += '<tr style="background:#f5f5f5;"><th style="padding:4px; text-align:left; font-size:10px;">Status</th><th style="padding:4px; text-align:right; font-size:10px;">Count</th></tr>';
        
        for(var i = 0; i < data.shipment_status.length; i++) {
            html += '<tr><td style="padding:4px; border-top:1px solid #eee;">' + data.shipment_status[i].Status + '</td>';
            html += '<td style="padding:4px; border-top:1px solid #eee; text-align:right;">' + data.shipment_status[i].ShipmentCount + '</td></tr>';
        }
        html += '</table>';
        document.getElementById('data-status').innerHTML = html;
    } else {
        document.getElementById('data-status').innerHTML = '<p style="color: #999;">No data</p>';
    }
    ...
    if(data.product_mix && data.product_mix.length > 0) {
        var html = '<p style="font-size: 10px; color: #666; margin-bottom: 5px; font-style: italic;">' + filterDesc + '</p>';
        html += '<table style="width:100%; border-collapse: collapse;">';
        html += '<tr style="background:#f5f5f5;"><th style="padding:4px; text-align:left; font-size:10px;">Product</th><th style="padding:4px; font-size:10px;">Category</th><th style="padding:4px; text-align:right; font-size:10px;">Units</th></tr>';
        
        // Show top 10 products only
        for(var i = 0; i < Math.min(10, data.product_mix.length); i++) {
            html += '<tr><td style="padding:4px; border-top:1px solid #eee;">' + data.product_mix[i].ProductName + '</td>';
            html += '<td style="padding:4px; border-top:1px solid #eee;">' + data.product_mix[i].Category + '</td>';
            html += '<td style="padding:4px; border-top:1px solid #eee; text-align:right;">' + data.product_mix[i].TotalUnits + '</td></tr>';
        }
        html += '</table>';
        document.getElementById('data-products').innerHTML = html;
    } else {
        document.getElementById('data-products').innerHTML = '<p style="color: #999;">No data</p>';
    }
    ...
    if(!hasDateRange) {
        document.getElementById('data-disruption-exposure').innerHTML = '<p style="color: #e67e22; font-size: 11px; font-weight: bold;">Please input a date range</p>';
    }
        ...
        var html = '<p style="font-size: 10px; color: #666; margin-bottom: 5px; font-style: italic;">From ' + startDate + ' to ' + endDate + ' and company ' + companyName + '</p>';
        ...
        html += '<div><strong>Total:</strong> <span style="font-size: 16px; color: #3498db; font-weight: bold;">' + data.disruption_exposure.total_disruptions + '</span></div>';
        html += '<div><strong>High Impact:</strong> <span style="font-size: 16px; color: #e67e22; font-weight: bold;">' + data.disruption_exposure.high_impact_events + '</span></div>';
        html += '<div><strong>Score:</strong> <span style="font-size: 18px; color: #e74c3c; font-weight: bold;">' + data.disruption_exposure.exposure_score + '</span></div>';
        html += '</div>';
        html += '<p style="font-size: 10px; color: #666; margin-top: 5px;">' + data.disruption_exposure.total_disruptions + ' + (2 x ' + data.disruption_exposure.high_impact_events + ') = ' + data.disruption_exposure.exposure_score + '</p>';
        html += '</div>';
    ...
    if(data.all_transactions && data.all_transactions.length > 0) {
        var html = '<p style="font-size: 10px; color: #666; margin-bottom: 8px; font-style: italic;">' + filterDesc + ' (showing up to 100 transactions)</p>';
        html += '<table style="width:100%; border-collapse: collapse; font-size: 10px;">';
        html += '<tr style="background:#f5f5f5;"><th style="padding:4px; text-align:left;">Type</th><th style="padding:4px; text-align:left;">Details</th></tr>';
        ...
        for(var i = 0; i < data.all_transactions.length; i++) {
            ...
            if(tx.Type === 'Shipping') {
                ...
                details += 'From: ' + tx.Shipping_SourceCompany + ' -> To: ' + tx.Shipping_DestinationCompany + '<br>';
                details += 'Product: ' + tx.Shipping_Product + ' (Qty: ' + tx.Shipping_Quantity + ')<br>';
                details += 'Promised: ' + tx.Shipping_PromisedDate;
                if(tx.Shipping_ActualDate) details += ' | Actual: ' + tx.Shipping_ActualDate;
            }
            else if(tx.Type === 'Receiving') {
                details = '<strong>Receipt #' + tx.Receiving_ReceivingID + '</strong> (Shipment #' + tx.Receiving_ShipmentID + ')<br>';
                details += 'Company: ' + tx.Receiving_Company + '<br>';
                details += 'Received: ' + tx.Receiving_ReceivedDate + ' (Qty: ' + tx.Receiving_Quantity + ')';
            }
                ...
                details += 'Date: ' + tx.Adjustment_Date + ' | Change: ' + tx.Adjustment_QuantityChange + '<br>';
                details += 'Reason: ' + tx.Adjustment_Reason;
            }
            
            html += '<tr><td style="padding:6px; border-top:1px solid #eee; vertical-align:top; width:80px;">' + tx.Type + '</td>';
            html += '<td style="padding:6px; border-top:1px solid #eee;">' + details + '</td></tr>';
        }
        
        html += '</table>';
        document.getElementById('data-all-transactions').innerHTML = html;
    } else {
        document.getElementById('data-all-transactions').innerHTML = '<p style="color: #999;">No transactions found with current filters</p>';
    }
    ...
    if(data.shipment_volume && data.shipment_volume.length > 0) {
        ...
        var distributorNames = data.shipment_volume.map(function(item) { return item.DistributorName; });
        var quantities = data.shipment_volume.map(function(item) { return item.TotalQty; });
    ...
    // =====================================================================
    if(data.delivery_rate && data.delivery_rate.length > 0) {
        var distributorNames2 = data.delivery_rate.map(function(item) { return item.DistributorName; });
        var rates = data.delivery_rate.map(function(item) { return parseFloat(item.DeliveryRate).toFixed(1); });
    ...
    // =====================================================================
    if(data.shipment_status && data.shipment_status.length > 0) {
        var statuses = data.shipment_status.map(function(item) { return item.Status; });
        var counts = data.shipment_status.map(function(item) { return item.ShipmentCount; });
    ...
    // =====================================================================
    if(data.product_mix && data.product_mix.length > 0) {
        var productNames = data.product_mix.map(function(item) { return item.ProductName; });
        var productUnits = data.product_mix.map(function(item) { return item.TotalUnits; });
...
function clearFilters() {
    ...
    document.getElementById('data-status').innerHTML = '<p style="color: #999;">No data</p>';
    document.getElementById('data-products').innerHTML = '<p style="color: #999;">No data</p>';
    document.getElementById('data-disruption-exposure').innerHTML = '<p style="color: #999; font-size: 10px;">Formula: Total + 2 x High Impact</p><p style="color: #999; font-size: 10px;">Select destination company and dates</p>';
    document.getElementById('data-all-transactions').innerHTML = '<p style="color: #999;">No data - Apply filters to see transactions</p>';
